...
def compute_job(queue : Queue, graph_name : str, job_id : str):
	counted = execute_job(graphs_dir + graph_name, jobs_dir+"poly20_big_" + str(job_id))
	queue.put((job_id,counted))
...
def main(batch_size):
	...
	sum = 0
	total_jobs = 760
	for job_id in range(0,total_jobs,batch_size):
		for i in range(batch_size):
			if job_id+i >= total_jobs:
				continue
			t = Thread(target=compute_job, args=(q, graph_name, str(job_id+i)))
			t.start()
			threads.append(t)
		for t in threads:
			t.join()
